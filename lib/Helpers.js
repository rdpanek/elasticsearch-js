...
class Helpers {
  ...
  bulk (options) {
    ...
    let currentFlushBytes = flushBytes
    const flushObserver = FlushObserver(flushBytes)
    ...
    async function iterate () {
      ...
      for await (const chunk of datasource) {
        ...
        flushObserver.incr(chunkBytes)
        bodyBytes += chunkBytes
        chunkBytes = 0
        if (bodyBytes >= currentFlushBytes) {
          stats.bytes += bodyBytes
          const send = await semaphore()
          send(bulkBody.slice())
          bulkBody.length = 0
          bodyBytes = 0
      ...
      // In some cases the previos http call does not have finished,
      // or we didn't reach the flush bytes threshold, so we force one last operation.
      if (shouldAbort === false && bodyBytes > 0) {
        const send = await semaphore()
        stats.bytes += bodyBytes
        send(bulkBody)
      }
    ...
    function FlushObserver (flushBytes) {
      if (flushBytes !== 'auto') {
        return { incr () {} }
      }

      currentFlushBytes = 5000000
      let doc = 1
      let meanSize = 1
      const minBytes = 1 // 1 byte
      const maxBytes = 100000000 // 10 mb

      const interval = setInterval(() => {
        // TODO: figure out a formula that takes into account
        //       documents, the mean size and the sample time
        currentFlushBytes = doc * meanSize
        if (currentFlushBytes < minBytes) currentFlushBytes = minBytes
        if (currentFlushBytes > maxBytes) currentFlushBytes = maxBytes
        doc = 1
        meanSize = 1
      }, 1000 * 10)
      ...
      return {
        incr (size) {
          ...
          meanSize = Math.round(meanSize + (size - meanSize) / doc)
        }
      }
    }
